...
- **删除归属语义**：仅移除注册、保留脏 id——落选：附件导入硬失败与工具根静默漂移两条路径行为分叉，续聊必踩其一；解析期统一软回退（保 id 不动、把 import_attachment 改为与工具链一致的默认回退）——落选：非破坏且视觉结果相同，但永久遗留无法解析的悬挂引用，要求未来每个 workspace_id 消费方都自带未知 id 处理，并削弱 import_attachment 对真正误传 id 的 fail-loud 姿态；重写 default 给出一次性收敛到合法指正的终点。禁删非空工作区——落选：把注册表管理耦合到会话生命周期，用户心智里"移出列表"不该被数据绑票。重写后与平铺区展示语义一致。
...
- 显示名仅 trim + ≤64 字符校验，不过滤控制字符/双向控制符（U+202E 等）：存储与渲染安全无虞（Vue 插值转义、参数化 SQL），存在 UI 显示异常与 RTL 命名欺骗空间，后续可白名单化。
- 多窗口场景未处理（现仅 main 窗口）。
