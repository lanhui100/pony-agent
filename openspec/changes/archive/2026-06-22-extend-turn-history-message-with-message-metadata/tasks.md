...
参见 [spec.md](./specs/extend-turn-history-message-with-message-metadata/spec.md)
...
1. `TurnHistoryMessage` 新增 `turn_id`、`status`（`Option<MessageStatus>` 枚举）、`model_name`、`token_count`、`reasoning_content` 字段（均为可选，向后兼容），附带 `stable_id()` 方法
