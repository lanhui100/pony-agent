...
主对话区在流式渲染期间需要保持 markdown 渲染能力，同时让流式呈现更平滑稳定：未闭合的围栏代码块通过流式专用部分渲染路径稳定呈现（不改动终态 transcript 真值）；流式文本继续使用字符/时间缓冲阈值控制可见更新节奏（首屏阈值更低、代码块内节奏更紧）；缓冲段跨越不安全 markdown 语法边界时回退到安全呈现模式；`prefers-reduced-motion` 下禁用或减弱动画；流式中消息显示进行中指示，操作控件仅在终态出现；优化支持运行时开关与可观测性，便于本地验证与回滚。
