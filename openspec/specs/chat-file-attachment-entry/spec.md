# chat-file-attachment-entry Specification

## Purpose

对话输入区（composer）需要支持文件附件：用户通过别针按钮选择文件，系统按数据驱动的类型注册表（扩展名优先、MIME 兜底、图片魔数嗅探）把文件路由为图片（多模态输入）、文本（内容注入，≤64KiB 截断）或二进制文档（引用附着，path+mime）；宿主负责把文件导入受控目录（`.tmp/imports/`），前端不直接写宿主文件系统；附件-only 发送自动生成摘要消息；待发送附件列表在发送、切换会话或新建会话时清空。本 spec 定义附件入口、类型注册表、处理路由与外部导入的完整合同。
