...
- **THEN** the request SHALL carry a stable `startReason` equivalent to `initial_turn`, `explicit_retry_after_failure`, `replay_from_checkpoint`, or `restart_from_checkpoint`
...
- **AND** it SHOULD model retry identity through `startReason` instead of a parallel retry-specific command family unless a concrete semantic gap is discovered
