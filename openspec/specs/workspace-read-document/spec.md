# workspace-read-document Specification

## Purpose

`workspace_read_document` 是内置工具面中 workspace-scoped 的办公文档读取能力：通过 Firecrawl `anydoc`（crates.io `=0.1.6`，纯 Rust、本地、MIT）把工作区内的 Word / PowerPoint / Excel / OpenDocument / RTF / EPUB / CSV / PDF 转换为 GitHub Flavored Markdown 返回给模型。它沿用 `view_image` 的安全模式（工作区路径约束、输入/输出字节上限、`truncated` 证据），通过 governed dispatcher 以 ModelVisible + `workspace.read` 注册执行，全程离线无外部服务（扫描版 PDF 需 OCR，明确不支持并 fail closed）。
