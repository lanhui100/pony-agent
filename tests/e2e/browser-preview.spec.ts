...
test("P0-E2E-002 浏览器预览模式可完成流式回放并落入历史", async ({ page }) => {
  ...
  await expect(page.getByTestId("session-switch-local-dev-session")).toBeVisible();
  await expect(page.getByTestId("session-sidebar-session-list")).toContainText("帮我解释浏览器预览模式");
});

test("P0-E2E-003 浏览器预览模式提交后保留可解释的 fallback 会话", async ({ page }) => {
  await openHome(page);
  ...
  await expect(page.getByTestId("workspace-submit-action")).toBeVisible();
  await expect(page.getByTestId("workspace-content-column")).toContainText("真正联调需要运行 tauri dev");
  await expect(page.getByTestId("session-sidebar-session-list")).toContainText("这次请在中途停止");
});
...
test("P0-E2E-004 页面刷新后会恢复已持久化会话", async ({ page }) => {
  ...
  await expect(page.getByTestId("workspace-composer-input")).toBeVisible();
  await expect(page.getByTestId("session-sidebar-session-list")).toContainText("刷新后也要看到这条会话");
  await expect(page.getByTestId("workspace-content-column")).toContainText("真正联调需要运行 tauri dev");
});
...
test("P0-E2E-005 可新建空白会话并切回历史会话", async ({ page }) => {
  ...
  await expect(page.getByTestId("session-sidebar-session-list")).toContainText("新对话");
  await expect(page.getByTestId("session-sidebar-session-list")).toContainText("未保存");
  await expect(page.getByTestId("session-sidebar-session-list")).toContainText("切换前保留这条历史");
...
test("P1-E2E-001 浏览器预览停止后的会话在刷新后仍保持可解释状态", async ({ page }) => {
  ...
  await openHome(page);
  await expect(page.getByTestId("workspace-content-column")).toContainText("真正联调需要运行 tauri dev");
  await expect(page.getByTestId("session-sidebar-session-list")).toContainText("刷新后也要保留停止态");
});
