...
- **Workspace 主线 PA-078~081 已全部收口**：PA-080（路径权限，commits `2db0b0e`/`a1b75d8`）；**PA-081（侧边栏树）已完成并收口（2026-08-22，提交 `fd0353d`）**——两级树 + 折叠持久化 + 跨项目落点修复，vitest 414+10skip 全绿；归档 `archive/2026-08-22-workspace-sidebar-tree-navigation/`，canonical `openspec/specs/workspace-sidebar-tree/spec.md`（注意与 `archive/2026-08-25-workspace-sidebar-three-level-tree/` 三级树演进区分）。
- **trace 面板性能四件套 PA-084~087 已完成并收口（2026-08-14）**，change 均已归档 `archive/2026-08-14-*`；**2026-09-16 canonical specs 按 as-built 重写同步**：`openspec/specs/{trace-panel-collapse,trace-panel-virtual-scroll,trace-render-snapshot,composer-input-priority}/spec.md`（085 turn 级虚拟化 / 086 memo helper 降级两处偏离已显式声明）。
- **PA-099 应用内更新检测已收口**：实现随 `c684d38` 落地、规范归档随 `b7ac018` 搬运（`archive/2026-08-24-add-app-update-check/` + canonical `openspec/specs/app-update-check/spec.md`，ADR 0012）。
- **PA-100 工具错误保真与文件分页已收口（2026-08-25）**：startLine 透传 + timeline 真实错误 + 429 长退避，core 913 全绿，提交 `7c41d73`。
- **PA-096 工作台布局 / PA-097 core 热点重构 / PA-098 runtime 拆分已收口**（任务板 Done 区）；Ready 区已清空（2026-09-16 收敛）。

- **新主线（2026-08-08 启动）：Workspace 多项目 + 对话文件附件**
  拆分为 4 张卡：`PA-078`（对话文件附件入口与类型白名单，P0，先行）→ `PA-079`（Workspace 数据模型与注册表）→ `PA-080`（Workspace 路径权限边界，P0，安全敏感）→ `PA-081`（侧边栏 Workspace 树导航）。每张卡走"任务卡 + OpenSpec change → 3 路对抗审核 → 采纳修订 → 实现 → 实现后 3 路审核 → 验证收口"闭环。对应 OpenSpec changes：`add-chat-file-attachment-entry` / `workspace-data-model-and-registry` / `workspace-path-permission-boundary` / `workspace-sidebar-tree-navigation`。**PA-078 已完成并收口；PA-079 已完成并收口（2026-08-09）**；spec 审核记录 `02_REVIEWS/2026-08-09-pa078-081-spec-review.md`，实现后审核 `2026-08-09-pa078-implementation-review.md` / `2026-08-09-pa079-implementation-review.md`。**2026-08-13 完成归档**：PA-078/079 的 OpenSpec changes 已迁入 `openspec/changes/archive/2026-08-09-{add-chat-file-attachment-entry,workspace-data-model-and-registry}/`，canonical specs 已同步（`openspec/specs/chat-file-attachment-entry/`、`openspec/specs/workspace-data-model/`）；同时收口归档 `optimize-agent-stream-ui` / `stabilize-markdown-stream-rendering` / `fix-provider-registry-fallback-test-baseline` 三个已完成 change（canonical specs：`chat-ui`、`provider-registry`）。**PA-080 实现完成（2026-08-13）**：`path_permission.rs` 统一路径判定 + 18 项对抗测试、授权清单持久化、宿主 authorize 命令、工具接入，验证 core lib 772 + 回归 13/5/8 + 前端 377 全绿（commit `2db0b0e`）。**实现后 3 路对抗审核 + 修复完成（2026-08-13）**：@code-reviewer 发现 P0 edit 写判定 / P1-1 Run cwd / P1-2 会话 root，@consultant 判定 FAIL 并指出 P0 会话 root 并发串扰 → 采纳方案 A（显式 `ToolExecutionContext` 贯穿工具链）+ 生产链路按 workspaceId 解析 root + 错误码结构化信封，全部修复（commit `a1b75d8`），最终验证 core lib 775 + 回归 13/5/8 + 前端 377 全绿。**PA-080 已收口归档（2026-08-13）**：change 迁入 `openspec/changes/archive/2026-08-13-workspace-path-permission-boundary/`，canonical spec `openspec/specs/workspace-path-permission/spec.md`。当前推进 **PA-081（侧边栏树）**。
